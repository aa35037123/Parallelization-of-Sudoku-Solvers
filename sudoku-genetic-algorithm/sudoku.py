    ...
    def solve(self):
        ...
        for generation in range(0, Ng):
            ...
            for count in range(Ne, Nc, 2):
                ...
                # print(f'#{count} child1 fitness: {child1.fitness}, child2 fitness: {child2.fitness}')
                child1.update_fitness() # should update fitness after crossover
                # Mutate child1.
                old_fitness = child1.fitness
                success = child1.mutate(mutation_rate, self.given)
                child1.update_fitness()
                # print(f'Updated child1 fitness: {child1.fitness}')
                if(success):
                    Nm += 1
                ...
                # Mutate child2.
                child2.update_fitness() # should update fitness after crossover
                old_fitness = child2.fitness
                success = child2.mutate(mutation_rate, self.given)
                child2.update_fitness()
                # print(f'Updated child2 fitness: {child2.fitness}')
                if(success):
                    Nm += 1
